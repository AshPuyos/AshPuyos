# Hola, soy Ayelén Puyos 👋

### Backend .NET Developer | C# | ASP.NET MVC | SQL Server | REST APIs | E-commerce

Soy **Analista Programadora** y actualmente trabajo como **.NET Developer en NextCommerce**, desarrollando y manteniendo soluciones e-commerce en producción.

Me especializo en **C#, ASP.NET MVC, .NET Framework, Entity Framework, SQL Server y APIs REST**, con experiencia en funcionalidades críticas como checkout, promociones, precios, órdenes, persistencia de datos e integraciones.

También trabajo con herramientas de **IA aplicada y automatización**, especialmente en flujos conversacionales, agentes para WhatsApp/Instagram e integración de APIs con plataformas de e-commerce.

Mi trayectoria previa en medios de comunicación incluyó roles de coordinación y gestión, participando en la planificación de programación, producción y supervisión de equipos. Esta experiencia fortaleció mis habilidades de liderazgo, comunicación, organización y relevamiento de necesidades, competencias que hoy aplico en proyectos de desarrollo de software, automatización e implementación de soluciones basadas en IA.

Cuento con **inglés avanzado/C1**, pasaporte europeo y experiencia trabajando de forma remota.

---

## 💻 Tech Stack principal

![C#](https://img.shields.io/badge/c%23-%23239120.svg?style=for-the-badge&logo=csharp&logoColor=white)
![.NET Framework](https://img.shields.io/badge/.NET%20Framework-5C2D91?style=for-the-badge&logo=.net&logoColor=white)
![ASP.NET MVC](https://img.shields.io/badge/ASP.NET%20MVC-512BD4?style=for-the-badge&logo=.net&logoColor=white)
![Entity Framework](https://img.shields.io/badge/Entity%20Framework-68217A?style=for-the-badge&logo=.net&logoColor=white)
![SQL Server](https://img.shields.io/badge/SQL%20Server-CC2927?style=for-the-badge&logo=microsoftsqlserver&logoColor=white)
![REST APIs](https://img.shields.io/badge/REST%20APIs-02569B?style=for-the-badge&logo=swagger&logoColor=white)

---

## 🌐 Frontend

![JavaScript](https://img.shields.io/badge/javascript-%23323330.svg?style=for-the-badge&logo=javascript&logoColor=%23F7DF1E)
![jQuery](https://img.shields.io/badge/jquery-0769AD?style=for-the-badge&logo=jquery&logoColor=white)
![HTML5](https://img.shields.io/badge/html5-%23E34F26.svg?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/css3-%231572B6.svg?style=for-the-badge&logo=css3&logoColor=white)
![Bootstrap](https://img.shields.io/badge/bootstrap-%238511FA.svg?style=for-the-badge&logo=bootstrap&logoColor=white)

---

## 🧠 También trabajo con

![Git](https://img.shields.io/badge/git-%23F05033.svg?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/github-%23121011.svg?style=for-the-badge&logo=github&logoColor=white)
![Python](https://img.shields.io/badge/python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54)
![AI Automation](https://img.shields.io/badge/AI%20Automation-111827?style=for-the-badge&logo=openai&logoColor=white)
![Claude Code](https://img.shields.io/badge/Claude%20Code-000000?style=for-the-badge&logo=anthropic&logoColor=white)

---

## 🚀 Experiencia actual

Actualmente en **NextCommerce**, trabajo sobre aplicaciones e-commerce desarrolladas en .NET, participando en:

- Desarrollo y mantenimiento de funcionalidades backend.
- Implementación de reglas de negocio para checkout, promociones, precios y órdenes.
- Integraciones mediante APIs REST.
- Consultas y optimización sobre SQL Server.
- Resolución de incidencias en entornos productivos.
- Automatizaciones y agentes conversacionales con IA para canales como WhatsApp e Instagram.

---

## 📌 Áreas que me interesan

- Backend development con .NET
- E-commerce
- APIs REST
- Automatización de procesos
- IA aplicada a productos reales
- Agentes conversacionales
- Integración entre sistemas
- Desarrollo asistido con IA

---

## 📫 Contacto

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Ayelén%20Puyos-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/ayelenpuyos/)

<!--
**AshPuyos/AshPuyos** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
